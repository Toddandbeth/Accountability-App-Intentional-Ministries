...
## Round 19: App Link Position, Group Icon Fix, Help & Tips Visual Redesign

### Move the app link's position in Settings

The copyable app link (added in Round 18) currently sits right after Profile, above Help & Tips. Move it instead to sit between Your Groups (including everything nested under it: code, meeting day, questions, members) and the Join/Create/Deactivate cluster at the bottom — a better match for when someone would actually reach for it.

### Group icon needs three people, not two

The Dashboard tab's new group icon (replacing the old gauge) currently shows two people, which reads ambiguously. Change to the standard three-person group composition — one figure forward/center, two behind — the more universally recognized "group" symbol. Keep the same size, thickness, and fillable/detailed style already achieved; only the number and arrangement of figures changes.

### Help & Tips — full visual redesign, applied consistently across all six topics

Three changes, all applied uniformly to every topic (not just the longer ones), so the whole section feels like one consistent design rather than a mix of styles:

1. **Small leading icon next to each of the 6 topic entries** on the main Help & Tips list, matching the iOS Settings pattern. Reuse existing icons where they already exist: Add to Home Screen uses a small version of the app's own logo mark, Check-in and Dashboard reuse their respective bottom-nav icons, Settings reuses the gear icon. Leader Guide uses a flag icon (confirmed preference). Questions & Answers uses a question mark in a circle.

2. **Grouped, rounded-rectangle "card" sections with divider lines** within each topic page, replacing plain scrolling text — applied to all six topics, not just the longer ones (Dashboard, Leader Guide), for visual consistency. Related subsections group into one rounded rectangle with divider lines between them (e.g. on Dashboard: "Reading the Dashboard" as its own card; "Opening a Member's Info," "Prayer & Life Updates," "Reactions," "Viewing History," "Viewing Goals" grouped together in one card with dividers; "Group Update" as its own separate card). Even shorter topics like Add to Home Screen should use this pattern (e.g. "On iPhone" and "On Android" as two natural groupings), so every topic page behaves the same way.

3. **Inline expand, not a further slide-over**, when tapping into a subsection within a topic page. Someone reaching a topic page is already two levels deep (Settings → topic); adding a third navigation layer for what's usually a short paragraph adds unnecessary tapping. This also matches the pattern already used and liked elsewhere in the app (Prayer & Life Update, Goals, Group Update all use inline expand for similar short-content reveals) — reuse that same interaction here rather than introducing a new one.
